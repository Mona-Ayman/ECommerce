...
namespace Domain.ProductRates
{
    public class ProductRate : BaseEntity<int>
    {
        ...
        public ProductRate(Guid productId, string userId, decimal rate)
        {
            ...
            AddPProductRatedEvent(ProductId);
        }
        ...
        public void ChechRateRange(decimal rate)
        {
            if (rate < 1 || rate > 5)
                throw new ValidationException(Localizations.RateRange);
        }

        private void AddPProductRatedEvent(Guid productId)
        {
            ProductRatedEvent productRatedEvent = new(productId);
